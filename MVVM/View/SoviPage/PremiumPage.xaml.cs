        ...
        Task.Delay(500, token).ContinueWith(async (task) =>
        {
            ...
            switch (nomeDoInvocador)
            {
                ...
                case "EntrySoviEisenbahn":
                    num = Math.Max(0, Math.Min(num, 100));
                    ((Entry)sender).Text = num.ToString();
                    SharedSoviInfos.PagesData[PageID].Produtos[(int)PremiumProducts.Eisenbahn].produtoSovi = (byte)num;
                    break;
                case "EntrySoviSol":
                    num = Math.Max(0, Math.Min(num, 100));
                    ((Entry)sender).Text = num.ToString();
                    SharedSoviInfos.PagesData[PageID].Produtos[(int)PremiumProducts.Sol].produtoSovi = (byte)num;
                    break;
            }
            ...
            for (int i = 0; i < 9; i++)
            {
                ...
                {
                    SharedSoviInfos.PagesData[PageID].Produtos[(int)PremiumProducts.Eisenbahn].produtoCentimetro[i] = num;

                    break;
                }
                else if (nomeDoInvocador == ("EntrySol" + indiceReal.ToString()))
                {
                    SharedSoviInfos.PagesData[PageID].Produtos[(int)PremiumProducts.Sol].produtoCentimetro[i] = num;
        ...
        var controlsToCheck = new List<(Entry entry, Func<int> getValue, Func<string> format)>()
{
    ...
    (EntrySoviEisenbahn, () => SharedSoviInfos.PagesData[PageID].Produtos[(int)PremiumProducts.Eisenbahn].produtoSovi, () => ""),
    (EntrySoviSol, () => SharedSoviInfos.PagesData[PageID].Produtos[(int)PremiumProducts.Sol].produtoSovi, () => ""),
};
